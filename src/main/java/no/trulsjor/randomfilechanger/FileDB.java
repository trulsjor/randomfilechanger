...
public class FileDB {
	...
	private List<String> invalidFileNames = Arrays.asList(".class", ".gif",
			".jpg", ".jpeg", ".png");
	...
	private void addFilesToDirectory(File directory) {
		for (File file : directory.listFiles()) {
			if (isValidDirectory(file)) {
				...
				db.put(db.size(),
						new FileEntry(db.size(), file.getAbsoluteFile()));
			}
		}
	}

	private boolean isValidFile(File file) {
		return file.isFile() && !invalidFileNames.contains(file.getName())
				&& !file.getName().startsWith(".");
	}

	private boolean isValidDirectory(File file) {
		return file.isDirectory()
				&& !invalidDirectories.contains(file.getName())
				&& !file.getName().startsWith(".");
	}
